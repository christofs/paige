...
def group_data(data, agg):
    grouped = data.groupby(by=["simple", agg]).median()
...
def plot_data_paige(grouped, filename): 
    untitled = grouped.query("simple == 'untitled'").loc["untitled",:]
    ...
    ax.set_title('Median lengths of novels by subtitle')
    ax.set_ylabel('Number of words (in thousands)')
    ax.set_xlabel('Time interval')
    #for i in range(len(labels)):
    #    ax.text(x[i]+0.0, y[i]+1.0, str(int(y[i])), size=8)
    ax.plot(x, y, label="untitled")

    histoire = grouped.query("simple == 'histoire'").loc["histoire",:]
    histoire.drop("1751-1830", axis=0, inplace=True)
    y = histoire["words"]
    #for i in range(len(labels)):
    #    ax.text(x[i]+0.0, y[i]+0.0, str(int(y[i])), size=8)
    ax.plot(x, y, label="histoire")

    nouvelle = grouped.query("simple == 'nouvelle'").loc["nouvelle",:]
...
def main(datafile, aggregations):
    ...
    for agg in aggregations: 
        ...
        filename = join(wdir, "fig4-4_paige-plot-"+agg+".png")
